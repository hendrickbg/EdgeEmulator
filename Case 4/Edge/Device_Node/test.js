...
async function getLastScAddress() {
  try {
      ...
      let latestBlockNumber = await privateProvider.getBlockNumber();
      ...
      while (latestBlockNumber >= startBlockNumber) {
          const block = await privateProvider.getBlockWithTransactions(latestBlockNumber);
          ...
          // Decrement the block number to scan the previous block
          latestBlockNumber--;
      }
...
// Function to scan for contract deployment events
async function scanForContractDeployment() {
  try {
...
async function node_fsm() {
  ...
  gtwContractAddress = await scanForContractDeployment();
  ...
  // gtwCatalogContract = await new ethers.Contract(gtwContractAddress, privContractAbi, privateProvider);
  // gtwCatalogContract.setOuputIpnsKey("123456");
  const result = await gtwCatalogContract.methods.catalogIpnsKey().call();
  console.log("Ipns Key: ", result);
