...
const HTTP_PORT_RASP = "9080" ;
const RASP_NAME = "device_node21";
...
async function getHttpPort() {
  const httoPortStr = HTTP_PORT_RASP; //process.env.HTTP_PORT;
  return httoPortStr;
}
...
async function importCatalogIpnsKeyName(ipfs) {
  ...
  const clone = "IpnsSecretKey_Rasp2";
  const keyFile = clone;
...
async function importOutputIpnsKeyName(ipfs) {
  ...
  const clone = "PrivateOutputIpnsKey_Rasp2";
  const keyFile = clone;
...
async function generateNodeIpnsKey(ipfs, deviceID) {
  ...
  await publishToIpns(ipfs, ipfs_cid, "IpnsSecretKey_Rasp2", 5000); 
}
...
async function importFarmMenuIpnsKeyName(ipfs) {
  ...
  const clone = "FarmMenuListSecretKey_Farm_Rasp2" + FARM_ID;
...
async function getFarmId() {
  const farmIdStr = FARM_ID;//process.env.FARM_ID;
...
async function getContainerName() {
  const containerName = RASP_NAME; //process.env.CONTAINER_NAME;
...
async function node_fsm() {
  ...
  FARM_ID = FAR_ID_RASP; //await getFarmId();
