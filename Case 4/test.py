...
def append_nodes(original_content, num_nodes, node_addr_list, farm_index):
    appended_content = original_content
    appended_content += "\n################################################################\n"

    for i in range(1, num_nodes + 1):
        new_service_name = increment_service_name(f"node{farm_index}", i)
        node_addr = node_addr_list[i - 1]
  ...
  {new_service_name}:
    ...
    command:
      ...
      --syncmode full
      --port {30320 + (farm_index * 10) + i}
      --http
      --http.addr 172.16.5.1
      --http.port {8547 + (farm_index * 10) + i}
      ...
      --authrpc.port {8548 + (farm_index * 10) + i}
      --http.corsdomain=*
      --networkid=1214
      --unlock 0x{node_addr}
    ...
    ports:
      - "{30320 + (farm_index * 10) + i}:{30320 + (farm_index * 10) + i}"
      - "{8547 + (farm_index * 10) + i}:{8547 + (farm_index * 10) + i}"
    network_mode: host
    extra_hosts:
...
def generate_ethereum_node(num_nodes, num_farms):
    global last_node_service

    for j in range(1, num_farms + 1):
        node_addr_list = []

        last_node_service = f"node{j}"
        ...
        for i in range(node_n + 1, num_nodes + 2):
            ...
            # to avoid overwriting the dockerfile related to the node1
            if i > 1:
        ...
        with open("docker-compose.yml", "r") as input_file:
            ...
            appended_docker_compose = append_nodes(original_content, num_nodes, node_addr_list, j)
...
def main():
    ...
    if action_select == 0:
        ...
        num_farms = int(input("Enter the number of farms: "))
        ...
        generate_ethereum_node(num_nodes, num_farms)
